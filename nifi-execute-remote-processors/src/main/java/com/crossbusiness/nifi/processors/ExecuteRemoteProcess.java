...
@CapabilityDescription("Runs an operating system command specified by the user on remote host and writes the output of that command to a FlowFile. If the command is expected "
...
public class ExecuteRemoteProcess extends AbstractProcessor {
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        try {
            ...
            log.info("Successfully executed remote command. flowFile {} ({}) in {} millis", new Object[]{flowFile, flowFile.getSize(), stopWatch.getElapsed(TimeUnit.MILLISECONDS)});
            session.transfer(flowFile, REL_SUCCESS);
        } catch (ProcessException | BadExitStatusException pe) {
            FlowFile flowFile = new NiFiUtils().exceptionToFlowFile(pe, session);
            log.error("Failed to execute remote command due to {}; routing to failure", new Object[]{pe.toString()}, pe);
